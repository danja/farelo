...
Generated by Repomix on: 2025-03-30T12:54:27.546Z
...
import config from '../../../config.json';  // Direct import of config file
...
export class EndpointManager {
    ...
    // New method to load directly from imported config
    loadFromConfig() {
        try {
    ...
    async loadFromFile() {
        try {
            // Now looking for config.json instead of endpoints.json
 ...
 */
export function initializeYasguiView() {
  ...
  try {
    ...
    // Get default endpoint object with credentials
    const defaultEndpoint = getDefaultEndpointObject();
    
    // Initialize YASGUI with endpoint config and auth
    yasgui = new Yasgui(container, {
      requestConfig: {
        endpoint: defaultEndpoint ? defaultEndpoint.url : 'http://localhost:4030/semem/query',
        // Add authentication through beforeSend callback
        beforeSend: (xhr) => {
          if (defaultEndpoint && defaultEndpoint.credentials) {
            const { user, password } = defaultEndpoint.credentials;
            const auth = btoa(`${user}:${password}`);
    ...
    try {
      configureEndpoints();
      state.subscribe('endpoints', configureEndpoints);
    } catch (configError) {
    ...
    showNotification('Error initializing SPARQL interface', 'error');
  }
}
...
function getDefaultEndpoint() {
  const endpoints = state.get('endpoints') || [];
  const queryEndpoint = endpoints.find(e => e.type === 'query' && e.status === 'active');
  return queryEndpoint ? queryEndpoint.url : 'http://localhost:4030/semem/query';
}
...
function getDefaultEndpointObject() {
  const endpoints = state.get('endpoints') || [];
  return endpoints.find(e => e.type === 'query' && e.status === 'active');
}
...
function configureEndpoints() {
  ...
  const endpoints = state.get('endpoints') || [];
  const queryEndpoints = endpoints.filter(e => e.type === 'query');
  ...
  try {
    ...
    // Get the YASQE instance from the tab
    const yasqe = tab.getYasqe();
    if (!yasqe) return;
    
    // Check if options and sparql properties exist before setting endpoints
    if (yasqe.options && yasqe.options.sparql) {
      // Configure endpoints with names and URLs
      yasqe.options.sparql.endpoints = queryEndpoints.map(e => ({
        name: e.label || e.url,
        endpoint: e.url
      }));
      
      // Set up authentication for endpoint requests
      yasqe.config.beforeSend = (xhr, url) => {
        // Find the endpoint that matches the URL
        const endpoint = queryEndpoints.find(e => url.includes(e.url));
        
        // If found and it has credentials, add auth header
        if (endpoint && endpoint.credentials) {
          const { user, password } = endpoint.credentials;
          const auth = btoa(`${user}:${password}`);
          xhr.setRequestHeader('Authorization', `Basic ${auth}`);
        }
      ...
      console.warn('YASQE options or sparql property is not defined');
    }
  } catch (error) {
...
document.addEventListener('routeChange', (e) => {
  if (e.detail.to === VIEWS.YASGUI) {
    // Ensure endpoints are initialized before showing YASGUI
    if (!state.get('endpoints') || state.get('endpoints').length === 0) {
      endpointManager.initialize().then(() => {
